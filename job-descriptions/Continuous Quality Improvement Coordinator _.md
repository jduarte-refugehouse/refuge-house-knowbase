## ![][image1]

# Continuous Quality Improvement (CQI) Coordinator

## Refuge House, Inc.

**Position Type:** Full-Time or Duty-Based Role Assignment  
**Reports To:** Executive Director  
**FLSA Status:** Exempt

---

## POSITION SUMMARY

The Continuous Quality Improvement (CQI) Coordinator oversees the systematic collection, analysis, and application of data to enhance service delivery and outcomes across all Service Packages. \[T3C\] This position fulfills the Continuous Quality Assurance and Improvement Program infrastructure requirement for all T3C Service Packages. \[TBRI\] Quality improvement is grounded in trauma-informed principles, ensuring that data-driven improvements support healing, connection, and positive outcomes for children and families.

---

## T3C INFRASTRUCTURE FUNCTION ALIGNMENT

This position fulfills the required T3C Infrastructure Function: **Continuous Quality Assurance and Improvement Program**

This function may be fulfilled through dedicated position, duty-based assignment to qualified staff, or contracted services.

---

## ESSENTIAL RESPONSIBILITIES

### CQI Program Development and Management

**Infrastructure and Systems Building**

* \[T3C\] Develop and maintain Continuous Quality Assurance and Improvement Program infrastructure for all Service Packages  
* Design data collection systems, dashboards, and automated reporting mechanisms  
* \[T3C\] Develop Logic Models for each Service Package  
* Create quality improvement plans aligned with agency goals and T3C requirements  
* Establish measurable quality indicators for child safety, permanency, and well-being  
* Build systems for real-time quality monitoring across programs and locations

**Program Coordination and Oversight**

* Coordinate quality improvement initiatives and facilitate improvement teams  
* Monitor outcomes by Service Package and foster home  
* Report quality metrics to leadership, Board of Directors, and regulatory bodies  
* Drive data-informed decision making across the agency  
* Support Service Package credentialing and expansion

### Data Systems and Outcome Measurement

**System Development and Data Analysis**

* Design and implement data collection systems in Radius and other platforms  
* Create dashboards and tools for tracking child-level outcomes  
* \[T3C\] Design Service Package-specific outcome tracking systems  
* Integrate CANS 3.0 assessment data into quality monitoring  
* Develop automated reporting for efficiency and accuracy

**Outcome Tracking and Analysis**

* \[T3C\] Track child outcomes for safety, permanency, and well-being by Service Package  
* Analyze placement stability, disruption patterns, and service effectiveness  
* \[TBRI\] Measure TBRI implementation fidelity  
* Monitor cost-effectiveness by Service Package  
* Identify trends, patterns, and best practices for replication  
* Translate complex data into actionable insights

### Compliance, Quality Assurance, and Improvement

**Compliance and Standards**

* Conduct internal quality audits  
* \[T3C\] Ensure Service Package compliance with T3C Blueprint requirements  
* Monitor documentation completeness and regulatory compliance (TAC §749, DFPS contracts, T3C standards)  
* \[TBRI\] Monitor trauma-informed care implementation quality  
* Prepare agency for external audits and credentialing reviews  
* Coordinate corrective action plans when deficiencies identified

**Performance Improvement**

* Identify improvement opportunities through data analysis  
* Develop and implement action plans addressing quality deficiencies  
* Evaluate effectiveness of quality improvement interventions  
* Research evidence-based practices and adapt to child welfare context  
* Foster culture of continuous improvement throughout organization

### Stakeholder Engagement and Reporting

**Feedback and Communication**

* Gather feedback from children, youth, young adults, foster parents, and staff  
* \[T3C\] Collect Service Package-specific input from stakeholders  
* Ensure youth voice in quality improvement decisions  
* Coordinate with external partners (DFPS, SSCC, community providers)

**Quality Reporting**

* Prepare quarterly quality reports for leadership and Board  
* \[T3C\] Report outcomes by Service Package to regulatory bodies  
* Present quality findings to staff and stakeholders  
* Communicate improvements, successes, and challenges transparently  
* Maintain audit-ready quality improvement documentation

**Capacity Building**

* Train staff on quality improvement methods, data collection, and Service Package outcome expectations  
* \[TBRI\] Support staff understanding of TBRI quality measures  
* Build organizational capacity for quality improvement  
* Create quality improvement resources and job aids

---

## REQUIRED QUALIFICATIONS

### Education and Experience

* Bachelor's degree in Social Work, Public Health, Business Administration, Information Systems, or related field (Required)  
* Master's degree in related field (Preferred)  
* Minimum 3 years experience in quality improvement, data analysis, or systems development  
* Child welfare experience strongly preferred  
* \[T3C\] Experience developing data systems and outcome measurement frameworks

### Knowledge, Skills, and Abilities

**Core Competencies**

* Strong data analysis and statistical skills  
* \[T3C\] Understanding of T3C Service Package requirements and outcome measurement  
* Proficiency in database management and data visualization tools  
* \[TBRI\] Knowledge of trauma-informed care principles  
* Project management and systems thinking abilities

**Essential Skills**

* Excellent written and verbal communication  
* Ability to translate complex data into actionable insights  
* Proficiency with technology platforms (Radius, Excel, data visualization tools)  
* Strategic planning and organizational development skills  
* Collaborative approach to quality improvement  
* Attention to detail and commitment to accuracy

**Preferred Qualifications**

* Quality improvement certification (CPHQ, Lean Six Sigma, or similar)  
* \[T3C\] Experience with Logic Model development  
* Database design and management skills  
* Experience with child welfare outcome measurement  
* IT systems development background

---

## ADDITIONAL REQUIREMENTS

### Training

* \[T3C\] Can be fulfilled by employee OR contracted specialist  
* Complete required training including:  
  * \[TBRI\] Trust-Based Relational Intervention (within 90 days)  
  * \[T3C\] T3C outcome measurement requirements  
  * Quality improvement methodologies  
  * Data analysis tools and software

### Employment Requirements

* Pass comprehensive background checks per TAC §749  
* Maintain confidentiality of sensitive data  
* Computer proficiency required  
* Reliable technology access for system development and monitoring

---

## WORKING CONDITIONS

* **Remote position** serving both Dallas and San Antonio locations  
* Primarily computer-based work building data systems and infrastructure  
* Flexible schedule with increased time during quarterly reporting periods  
* Minimum 20 hours per week for CQI activities (may be higher during implementation)  
* Periodic virtual meetings with leadership, staff, and stakeholders  
* Occasional travel to Dallas and San Antonio locations for site visits as needed  
* Capability to present to groups virtually (Board, staff, stakeholders)

---

## IMPLEMENTATION NOTES FOR T3C CREDENTIALING

### During Implementation Phase:

This infrastructure function may be fulfilled through duty-based assignment to qualified staff with systems development and data analysis expertise. During implementation, the assigned individual develops comprehensive CQI infrastructure including data systems, outcome tracking tools, reporting mechanisms, Logic Models, and quality improvement processes designed for long-term organizational sustainability.

### System-Building Focus:

The CQI Coordinator role during implementation emphasizes **building CQI infrastructure** rather than simply operating established systems. This includes designing data collection and analysis systems, creating dashboards and automated reporting tools, developing Logic Models for each Service Package, building outcome tracking mechanisms, establishing quality improvement processes and procedures, and creating training materials and resources.

### Long-Term Sustainability:

Once CQI systems are built and operational, this function may transition to Program Director, dedicated Quality Improvement staff, or remain with operations leadership depending on organizational needs and capacity.

---

## PERFORMANCE METRICS

**Quality Program Outcomes:**

* CQI infrastructure development and implementation  
* Data system functionality and user adoption  
* Quality improvement initiative completion rates  
* Achievement of outcome improvement goals

**Process Measures:**

* Timeliness of quality reports (quarterly)  
* Internal audit completion rates  
* Staff training on quality improvement methods  
* \[T3C\] Service Package outcome data accuracy

**Quality Indicators:**

* Placement stability rates by Service Package  
* TBRI implementation fidelity scores  
* Regulatory compliance rates  
* Stakeholder satisfaction with quality processes  
* \[T3C\] Child safety, permanency, and well-being outcomes by Service Package

---

## ORGANIZATIONAL RELATIONSHIPS

**Reports To:** Executive Director

**Collaborates With:**

* Program Directors (Dallas and San Antonio)  
* Treatment Directors  
* IT Administrator/Technical Development  
* All direct service staff  
* Foster parents and caregivers  
* External evaluators and auditors

---

## EQUAL OPPORTUNITY EMPLOYER

Refuge House San Antonio, Inc. is an equal opportunity employer and does not discriminate on the basis of race, color, religion, sex, national origin, age, disability, veteran status, sexual orientation, gender identity, or any other protected characteristic.

---

**Date Created:** \[To Be Completed\]  
**Date Last Revised:** \[To Be Completed\]  
**Approved By:** Michele L. Gorman, Ph.D., Executive Director

---

## NOTES FOR T3C INFRASTRUCTURE FUNCTION VERIFICATION

This job description fulfills the required T3C Infrastructure Function: **Continuous Quality Assurance and Improvement Program**

**Current Implementation:**

* **Staff Assignment:** Jeannie Duarte, Director of Operational Development (Co-Founder)  
* **Work Location:** Fully remote serving both Dallas and San Antonio locations  
* **Implementation Focus:** Building comprehensive CQI infrastructure including Radius dashboards, data collection systems, outcome tracking tools, Logic Models, reporting mechanisms, and quality improvement processes  
* **Service Coverage:** Both Dallas and San Antonio locations  
* **Expertise Applied:** IT systems development, data analysis, process improvement, and organizational development  
* **Timeline:** CQI infrastructure development ongoing throughout interim credential phase

This duty-based assignment meets T3C Blueprint requirements by leveraging the Director of Operational Development's systems architecture expertise to build sustainable, scalable CQI infrastructure appropriate to agency size and service capacity.

**System-Building Approach:** During implementation, the CQI Coordinator role emphasizes creating the technological and procedural infrastructure that enables continuous quality improvement. This includes designing data systems, building automated reporting tools, developing outcome measurement frameworks, and establishing processes that can be scaled as the organization grows. Remote work is ideal for this systems development focus, allowing concentrated time for complex systems architecture without the overhead of office-based work. This approach ensures long-term sustainability by building robust systems rather than relying solely on manual processes.  


[image1]: <data:image/png;base64,>