## ![][image1]

# KINSHIP SUPPORT SERVICES IMPLEMENTATION ROLE

## ROLE OVERVIEW

During the implementation phase, the Kinship Support Services program will be launched and administered by ONE designated staff member who holds an existing leadership position (Program Director, Treatment Director, or Home Developer). This integrated approach ensures efficient implementation without creating unsustainable complexity. Even after transitioning to a standalone position, these leaders will provide backup coverage as needed.

## IMPLEMENTATION PHASE STRUCTURE

### Single Point of Accountability

**ONE designated leader will:**

- Launch the kinship support program  
- Manage initial referrals and cases  
- Develop and refine processes  
- Track success metrics  
- Make program adjustments based on data

### Time Allocation

- **Months 1-3**: Approximately 15-20 hours per week  
- **Months 4-6**: Approximately 20-25 hours per week  
- **Months 7-12**: Evaluate need for dedicated position based on volume and outcomes

### Backup Coverage

Other leadership team members (Program Director, Treatment Director, Home Developer) will be cross-trained to:

- Provide coverage during absences  
- Handle overflow when caseloads peak  
- Support crisis situations  
- Share expertise as needed

## IMPLEMENTATION PHASE PRIORITIES

### Primary Focus: Launch and Learn (60% of effort)

- Accept and manage initial referrals (start with 3-5 cases)  
- Establish basic processes that work  
- Test and refine simple tools  
- Build relationships with DFPS/SSCC  
- Document what works and what doesn't

### Secondary Focus: Measure and Adjust (30% of effort)

- Track essential metrics:  
  - Referral response times  
  - Verification completion rates  
  - Placement stability  
  - Caregiver satisfaction  
- Analyze monthly data  
- Adjust processes based on findings  
- Report outcomes to leadership

### Tertiary Focus: Build for Growth (10% of effort)

- Create simple resources for families  
- Develop basic training materials  
- Plan for gradual expansion  
- Prepare for eventual dedicated position

## SIMPLIFIED RESPONSIBILITIES

### Month 1-3: Foundation

- Respond to referrals within 24 hours  
- Complete basic assessments  
- Support 3-5 kinship families  
- Track time and activities  
- Identify what's working

### Month 4-6: Refinement

- Expand to 7-10 families  
- Streamline successful processes  
- Eliminate ineffective activities  
- Begin monthly support group  
- Measure early outcomes

### Month 7-12: Evaluation and Growth

- Maintain 10-15 families  
- Analyze program effectiveness  
- Recommend staffing needs  
- Prepare transition plan  
- Document best practices

## QUALIFICATIONS FOR IMPLEMENTATION LEADER

### Required

- Current leadership position (Program Director, Treatment Director, or Home Developer)  
- Understanding of foster care system  
- Basic data tracking skills  
- Flexibility to adjust approach based on learning

### Helpful but Not Required

- Previous kinship care experience  
- Benefits knowledge (can be learned)  
- TBRI® training (can be obtained during implementation)

## SUCCESS METRICS FOR IMPLEMENTATION

### Minimum Compliance (Must Achieve)

- 100% referral response within 24 hours  
- 100% basic documentation completed  
- Zero licensing citations

### Quality Indicators (Track and Improve)

- Verification completion rate  
- Placement stability at 3 and 6 months  
- Caregiver satisfaction ratings  
- Time per case (efficiency measure)

### Program Viability (Evaluate Monthly)

- Number of referrals received  
- Staff time required per case  
- Cost per placement supported  
- Outcome improvements demonstrated

## TRANSITION PLANNING

### Months 1-6: Proof of Concept

- Can one person effectively manage the program?  
- What is the actual time requirement?  
- Are outcomes improving?  
- Is the model sustainable?

### Months 7-9: Staffing Analysis

- Calculate cases per FTE  
- Project future referral volume  
- Determine optimal staffing model  
- Plan for dedicated position if warranted

### Months 10-12: Implementation of Long-term Model

- Transition to dedicated position OR  
- Formalize integrated model with clear backup  
- Document standard operating procedures  
- Ensure continuity plan

## KEY PRINCIPLES FOR SUCCESS

1. **Keep It Simple**: Complex processes fail. Simple processes succeed.  
2. **Measure What Matters**: Focus on 3-5 key metrics, not 30\.  
3. **Adjust Quickly**: If something isn't working by month 2, change it.  
4. **Document Briefly**: One-page summaries are better than lengthy reports.  
5. **Celebrate Progress**: Small wins build momentum.

## SUPPORT FOR IMPLEMENTATION LEADER

- Weekly check-in with Executive Director (15 minutes)  
- Monthly data review with leadership team  
- Authority to adjust processes without approval  
- Protected time for kinship duties  
- Recognition that this is a learning process

---

*This role description emphasizes practical implementation over theoretical perfection. The goal is to launch a sustainable program that improves outcomes for kinship families while building evidence for future expansion.*  


[image1]: <data:image/png;base64,>