## ![][image1]

# Human Resources & Quality Assurance Director (HRQAD)

**Position Type:** Full-Time, Exempt  
**Reports To:** Executive Director  
**Location:** Dallas, TX (with San Antonio responsibilities)  
**FLSA Status:** Exempt

---

## POSITION SUMMARY

The Human Resources & Quality Assurance Director provides strategic leadership and operational oversight for all human resources functions, staff training and workforce development, quality assurance, and continuous quality improvement initiatives across both Dallas and San Antonio locations. This position ensures organizational compliance with T3C System Blueprint requirements, regulatory standards, and best practices in child welfare services.

---

## T3C INFRASTRUCTURE FUNCTION ALIGNMENT

This position fulfills two of the eight required T3C Infrastructure Functions:

1. **Staff Training and Workforce Development**  
2. **Staff Recruitment and Retention**

---

## ESSENTIAL RESPONSIBILITIES

### Staff Training and Workforce Development (T3C Infrastructure Function)

**Training Program Management**

* Develop, procure, or coordinate comprehensive training curriculum aligned with TBRI Treatment Model and T3C requirements  
* Ensure all staff receive required orientation, pre-service, and annual continuing education  
* Coordinate service package-specific training for specialized roles  
* Maintain training documentation and compliance tracking  
* Evaluate training effectiveness and implement improvements

**Workforce Development**

* Assess organizational training needs and create annual training plans  
* Coordinate individual development plans with supervisors  
* Partner with external training providers and credentialing organizations  
* Monitor training compliance and reporting requirements  
* Support career advancement and succession planning

**Policy and Compliance**

* Develop and maintain training-related policies and procedures  
* Ensure alignment with T3C Blueprint, TAC Chapter 749, and DFPS requirements  
* Maintain audit-ready training records and documentation

### Staff Recruitment and Retention (T3C Infrastructure Function)

**Recruitment Planning and Execution**

* Develop and implement recruitment strategies for all staff positions  
* Create recruitment materials and manage advertising across multiple platforms  
* Screen candidates, coordinate interviews, and verify credentials  
* Establish talent pipelines for specialized T3C roles  
* Build partnerships with universities and professional associations

**Retention and Performance Management**

* Design and implement retention strategies to maintain stable workforce  
* Oversee performance management processes and evaluation cycles  
* Address turnover trends and develop targeted retention initiatives  
* Coordinate recognition programs and employee engagement activities  
* Conduct exit interviews and analyze retention data

**Onboarding and Integration**

* Design and oversee comprehensive onboarding program  
* Facilitate new employee integration into organizational culture  
* Monitor new employee engagement and retention during initial employment period

### Quality Assurance and Continuous Quality Improvement

**Compliance and Quality Monitoring**

* Monitor organizational compliance with HR-related regulatory requirements  
* Ensure personnel files meet TAC Chapter 749 minimum standards  
* Participate in agency-wide CQI committees and initiatives  
* Coordinate internal audits and prepare for external inspections

**Data Systems and Reporting**

* Coordinate with Technical Development team on HRIS implementation  
* Develop and track HR and training key performance indicators  
* Prepare regular reports on recruitment, retention, and training outcomes  
* Support data-driven decision making and performance improvement

### General Human Resources Functions

**HR Administration**

* Serve as primary HR contact for employee questions and concerns  
* Administer HR policies including leave, benefits, and time off  
* Investigate and resolve employee relations issues  
* Ensure fair and consistent application of policies across the organization

**Compliance and Benefits**

* Ensure compliance with federal and state employment laws (EEOC, ADA, FMLA, FLSA)  
* Coordinate benefits enrollment and administration  
* Maintain confidential personnel files in compliance with regulations  
* Support compensation planning and market competitiveness

---

## COORDINATION WITH TECHNICAL DEVELOPMENT TEAM

Works closely with Technical Development team to implement and optimize HRIS, training tracking systems, automated reporting dashboards, and data visualization tools supporting HR, training, and CQI functions.

---

## REQUIRED QUALIFICATIONS

### Education and Experience

* Bachelor's degree in Human Resources, Education, Social Work, Business Administration, or related field; OR  
* Minimum 5 years of progressive human resources experience with demonstrated competency in training coordination, recruitment, and workforce management  
* Master's degree in related field (Preferred)  
* Minimum 3 years supervisory or management experience  
* Experience in nonprofit or child welfare setting (Preferred)

### Certifications and Credentials

* PHR, SPHR, SHRM-CP, or SHRM-SCP certification (Preferred)  
* CPTD (Certified Professional in Training and Development) or similar training certification (Preferred)

### Knowledge, Skills, and Abilities

**Core Competencies**

* Comprehensive understanding of HR best practices and employment law  
* Knowledge of training coordination and workforce development principles  
* Understanding of child welfare systems and trauma-informed care  
* Familiarity with T3C System Blueprint requirements (or ability to learn quickly)  
* Proficiency in HRIS and training management systems  
* Strong recruitment and talent acquisition capabilities  
* Data analysis and reporting skills  
* Project management abilities

**Essential Skills**

* Strategic thinking and planning  
* Excellent written and verbal communication  
* Strong organizational and multitasking capabilities  
* Ability to maintain confidentiality  
* Cultural competency and inclusive practice  
* Collaborative approach to cross-functional work  
* Problem-solving and critical thinking  
* Adaptability to changing priorities

---

## WORKING CONDITIONS AND PHYSICAL REQUIREMENTS

* Primary office location in Dallas with regular coordination of San Antonio operations  
* Regular travel to San Antonio location required (frequency based on organizational needs)  
* Hybrid work arrangement with remote capabilities for cross-location coordination  
* Occasional evening or weekend work for training delivery or recruitment events  
* Regular computer use and ability to sit for extended periods  
* Ability to lift and carry training materials (up to 25 lbs occasionally)

---

## PERFORMANCE METRICS

**Training and Workforce Development:**

* 100% compliance with required training for all staff  
* Training completion rates and on-time completion  
* Training effectiveness and participant satisfaction scores

**Recruitment and Retention:**

* Time-to-fill for open positions  
* Staff retention rates and turnover reduction  
* Quality of hire metrics  
* New employee satisfaction scores

**Quality Assurance:**

* HR audit readiness and compliance scores  
* Data accuracy and system integrity metrics  
* CQI participation and improvement implementation

**General HR:**

* Employee satisfaction and engagement scores  
* Timely resolution of employee relations issues  
* Personnel file compliance and audit readiness

---

## ORGANIZATIONAL RELATIONSHIPS

**Reports To:** Executive Director

**Collaborates With:**

* Executive Leadership Team  
* Program Directors (Dallas and San Antonio)  
* Technical Development Team  
* All department managers and supervisors  
* External training providers and vendors

---

## EQUAL OPPORTUNITY EMPLOYER

Refuge House San Antonio, Inc. is an equal opportunity employer and does not discriminate on the basis of race, color, religion, sex, national origin, age, disability, veteran status, sexual orientation, gender identity, or any other protected characteristic.

---

## NOTES FOR T3C INFRASTRUCTURE FUNCTION VERIFICATION

This job description fulfills two required T3C Infrastructure Functions:

1. **Staff Training and Workforce Development** \- See Essential Responsibilities section  
2. **Staff Recruitment and Retention** \- See Essential Responsibilities section

This position ensures Refuge House maintains identified personnel and infrastructure to support these critical T3C System Blueprint requirements across all service packages.  


[image1]: <data:image/png;base64,>