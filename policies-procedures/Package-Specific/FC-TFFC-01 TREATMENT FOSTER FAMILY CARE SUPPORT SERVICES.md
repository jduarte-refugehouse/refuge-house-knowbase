# REFUGE HOUSE, INC.

**Child Placing Agency**

---

## T3C TREATMENT FOSTER FAMILY CARE SUPPORT SERVICES

## UMBRELLA POLICY AND PROCEDURES

| Policy Information | Details |
| :---- | :---- |
| **POLICY NAME** | T3C Treatment Foster Family Care Support Services Policy |
| **POLICY NUMBER** | FC-TFFC-01 |
| **ORIGINATED** | December 2025 |
| **APPROVED BY** | Board of Directors |
| **APPROVED ON** | [Date] |
| **EFFECTIVE DATE** | [Date] |
| **SECTION** | FC-TFFC |
| **DATE(S) OF REVISION** | |

| APPLICABLE T3C PACKAGES: | APPLICABLE T3C ADD-ON SERVICES: |
| :---- | :---- |
| ☐ T3C Basic Foster Family Home | ☒ Transition Support Services for Youth & Young Adults |
| ☐ Substance Use Support Services | ☒ Kinship Caregiver Support Services |
| ☐ Short-Term Assessment | ☒ Pregnant & Parenting Youth or Young Adult |
| ☐ Mental & Behavioral Health | |
| ☐ Sexual Aggression/Sex Offender | |
| ☐ Complex Medical Needs or Medically Fragile | |
| ☐ Human Trafficking Victim/Survivor | |
| ☐ Intellectual or Developmental Disability (IDD)/Autism Spectrum Disorder | |
| ☒ **T3C Treatment Foster Family Care** | |

---

## DOCUMENT OVERVIEW

This umbrella policy aggregates all requirements for T3C Treatment Foster Family Care Support Services, demonstrating how this Service Package integrates with Refuge House's established policy and procedure infrastructure. This is the **highest level of clinical intervention available in a family setting**.

**Legal Framework:** Services adhere to Texas Family Code Section 264.1073 and Texas Administrative Code Rule §700.1335.

**Integration Framework:** This document works in conjunction with:
- **FC-T3C-01** T3C Basic Foster Family Home Support Services Policy (master T3C framework)
- **FC-MH-01** Mental & Behavioral Health Support Services Policy (clinical framework)
- **FC-04** Crisis Management Policy (high-acuity crisis principles)
- **FC-04.1** Crisis Management Procedure Section 6 (Treatment Foster Family Care crisis protocols)
- **FC-16** Staff and Caregiver Training Policy Section 4.6 (TFFC training requirements)
- **FC1-01.1** Admission Screening Procedure
- **FC2-01.1** Admission Assessment Procedure
- **FC3-01.1** Individual Service Planning Procedure
- **FC6-01** Admission Matching & Criteria Policy
- **FC14-01** Discharge and Permanency Planning Policy
- **FC-AF-01** Aftercare Services Policy
- **FC-CSR-01.1** Continued Stay Review Procedure
- **FC-STAR-01.1** STAR Health Coordination Procedure
- **FC-CRED-01.1** Foster Family Home Credentialing Procedure

---

## TABLE OF CONTENTS

**POLICY SECTIONS**
1. Purpose and Service Package Description
2. Governing Principles
3. Admission Criteria and Guidelines
4. Length of Service Parameters
5. Service Package Expectations
6. Staffing Requirements and Ratios
7. Foster Family Home Requirements
8. Quality Assurance and Continued Stay Guidelines
9. Aftercare Services Requirements

**PROCEDURE SECTIONS**
10. Relationship Between Policy and Procedure
11. Admission and Placement Procedures
12. Service Planning Procedures
13. 60-Day Continued Stay Review Procedures
14. Therapeutic Services Procedures
15. Discharge and Step-Down Procedures
16. Aftercare Services Procedures

**TREATMENT MODEL AND CQI**
17. Treatment Model Application
18. Continuous Quality Improvement

**REFERENCE SECTIONS**
19. Definitions
20. Regulatory References
21. Related Policies and Procedures
22. Forms and Attachments

---

# POLICY SECTIONS

## 1. PURPOSE AND SERVICE PACKAGE DESCRIPTION

### 1.1 Purpose

This policy establishes Refuge House's commitment to providing the highest level of clinical intervention in a family setting under the T3C Treatment Foster Family Care Support Services Package.

### 1.2 Service Package Description

Treatment Foster Family Care provides intensive, time-limited therapeutic services for children with serious emotional, conduct, or behavioral disorders who require concentrated clinical intervention but can be safely served in a specialized foster home.

**Target Population:** Children who meet admission criteria demonstrating need for highest level of clinical intervention available in a family setting to perform day-to-day activities.

### 1.3 Relationship to Other Service Packages

This package represents the highest intensity of foster care services. Children may step down to:
- T3C Basic Foster Family Home Support Services (same home if dually credentialed, or different home)
- Other appropriate Service Package
- Permanency placement

---

## 2. GOVERNING PRINCIPLES

### 2.1 TBRI® Foundation with Intensive Clinical Enhancement

**Cross-Reference:** Refuge House implements TBRI® as our Evidence-informed Treatment Model per **FC-T3C-01 T3C Basic Foster Family Home Support Services Policy** and **TBRI Treatment Model Executive Summary**.

For Treatment Foster Family Care, TBRI principles are enhanced with intensive clinical interventions:

**Connecting Principles:** Intensive relationship-building to address severe attachment disruptions and trust deficits resulting from complex trauma.

**Empowering Principles:** Comprehensive regulation support for children with severe emotional dysregulation, including crisis prevention and intensive behavioral support.

**Correcting Principles:** Highly structured, therapeutic environment with immediate access to clinical intervention when behavioral escalation occurs.

### 2.2 Time-Limited Treatment Philosophy

This package recognizes that intensive intervention should achieve stabilization within defined timeframes, with step-down planning integrated from Day 1.

---

## 3. ADMISSION CRITERIA AND GUIDELINES

### 3.1 Admission Eligibility

**Primary Requirement:** DSM-5 diagnosis for emotional, conduct, or behavioral disorder

**PLUS Two or More of the Following:**
- Major self-injurious actions, including suicide attempt within last 12 months
- Difficulties presenting significant risk of harm to others, including frequent or unpredictable physical aggression
- Additional DSM-5 diagnosis of substance-related and/or addictive disorder with severe impairment

### 3.2 Admission Process

**Cross-Reference:** Admission follows **FC1-01.1 Admission Screening Procedure** and **FC2-01.1 Admission Assessment Procedure** with the following package-specific requirements:

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| Treatment Director | 1. Review clinical documentation 2. Verify DSM-5 diagnosis 3. Confirm child meets 2+ additional criteria 4. Document clinical appropriateness | Before placement | T3C Blueprint p.141-142; FC2-01.1 |

**Per FC6-01 Admission Matching & Criteria Policy** with additional requirements:
- Child Placing Agency AND Foster Family Home must be Credentialed for Treatment Foster Family Care
- Dual credentialing verified (Treatment Foster Care AND Basic Foster Home)
- Home capacity verified (maximum 2 children)
- Pre-placement visit conducted when appropriate

### 3.3 Capacity Limitation

**CRITICAL:** Foster homes providing Treatment Foster Family Care may have **no more than two (2) children in foster care** at the same time due to the intensive nature of services.

---

## 4. LENGTH OF SERVICE PARAMETERS

### 4.1 Time-Limited Service

| Parameter | Duration |
|-----------|----------|
| Standard Treatment Period | Up to 274 days |
| Extension (if clinically necessary) | Up to 91 additional days |
| **Maximum Total** | **365 days** |

**CRITICAL:** An individual child cannot be served under this package for more than 365 days.

### 4.2 Anticipated Length of Service

Refuge House policy must include anticipated Length of Service for children served under this package, recognizing that intensive intervention should achieve stabilization for step-down within defined timeframes.

### 4.3 Step-Down Planning from Day 1

Due to time-limited nature, step-down planning begins at admission with preliminary step-down goals included in initial Service Plan.

---

## 5. SERVICE PACKAGE EXPECTATIONS

### 5.1 Therapeutic Services

Child must receive **regular and frequent** therapeutic services:
- Individual therapy (weekly minimum, frequency per clinical need)
- Family therapy (as clinically indicated)
- Group therapy (as clinically indicated)

**Cross-Reference:** Services coordinated through **FC-STAR-01.1 STAR Health Coordination Procedure**. Therapy frequency must be justified in writing in Service Plan.

**Therapist Qualifications:** Licensed Therapist specializing in serious mental, emotional, and/or behavioral disorders.

### 5.2 Service Planning Requirements

**Cross-Reference:** Service planning follows **FC3-01.1 Individual Service Planning Procedure** with the following package-specific modifications:

| Requirement | Treatment Foster Care | Standard (per FC3-01.1) |
|-------------|----------------------|-------------------------|
| Service Plan Reviews | **Every 60 days** | Every 90 days (MH/IDD) |
| CANS 3.0 | Every 90 days | Every 90 days |
| Step-Down Assessment | At each 60-day review | Not applicable |

#### Youth-Informed Service Planning

Service Plans must be informed by the child, youth, or young adult (when age-appropriate) and developed in collaboration with the Service Planning team. Each Service Plan includes:

- **Customized goals** developed with meaningful youth input that reflect their voice, perspective, and priorities
- **Specific services and supports** tailored to achieve each goal
- **Documentation of youth participation** in the goal-setting and planning process
- **Cultural considerations** that honor the youth's identity and values

The Service Planning team incorporates input from the child/youth, Case Manager, Treatment Director, Licensed Therapist, Foster Parents, SSCC/DFPS caseworker, biological family/support persons (when appropriate), educational representatives, and medical providers (as applicable).

**T3C Blueprint Reference:** TFFC Support Services p.138 (October 2025) - Service Plan must be informed by child/youth and include customized goals and planned services to achieve those goals.

#### Service Plan Review - Progress Documentation

Service Plan reviews must include documentation demonstrating the progress made toward achieving **each goal** identified in the Service Plan. Progress documentation is:

- **Goal-specific** - addressing each stated goal individually, not general observations
- **Measurable** - using objective indicators where possible (frequency, duration, intensity)
- **Descriptive** - providing specific examples and observations that illustrate progress or challenges
- **Action-oriented** - identifying next steps, adjustments, and continued supports needed

For each goal in the Service Plan, the review documents:
- Current status of goal achievement (no progress, minimal progress, moderate progress, significant progress, goal achieved)
- Specific examples demonstrating current functioning related to the goal
- Effective interventions and supports that have promoted progress
- Barriers to progress and challenges encountered
- Adjustments needed to services, supports, or the goal itself

**Review Frequency:** Service Plan reviews occur **every 60 days** for TFFC Support Services, reflecting the intensive clinical intervention and time-limited nature of this package.

**T3C Blueprint Reference:** TFFC Support Services p.138 (October 2025) - Service Plan reviews must include documentation to show the progress made toward achieving each goal.

**Cross-Reference:** FC3-01.1 Individual Service Planning Procedure Section 3 (Service Plan Reviews)

#### CANS 3.0 Assessment Requirements

**T3C Blueprint Reference:** TFFC Support Services p.139 (October 2025)

Children over the age of 3, youth, and young adults must receive CANS 3.0 Assessments according to the following schedule:

**Initial Assessment:**
- Children ages 3-5: Within 21 days of placement
- Children ages 6 and older: Within 30 days of placement

**Ongoing Assessments:**
- **Every 90 days** thereafter for the duration of placement

**Children Age 3 and Under:**
- CANS 3.0 not required but may be completed if clinically indicated

**TFFC-Specific Considerations:**
- Given intensive clinical intervention and 60-day Service Plan reviews, CANS 3.0 provides critical data for treatment adjustments
- Results analyzed in conjunction with 60-day reviews to assess progress and continued need for TFFC-level services
- Changes in CANS 3.0 domains inform step-down planning and transition readiness

**Assessment Administration:**
- Conducted by licensed clinical staff certified in CANS 3.0 administration (LMSW, LPC, LCSW, LPC-S, LMFT)
- Treatment Director or designee coordinates completion
- Case Manager ensures scheduling and follow-through

**Integration with Intensive Services:**
- Results directly inform Service Plan development and modifications
- CANS 3.0 findings guide adjustments to therapy frequency and intensity
- Progress across domains tracked to evaluate effectiveness of intensive intervention
- Regression in domains triggers immediate clinical review and service adjustment

**Documentation:**
- CANS 3.0 results entered into designated system
- Summary of findings in case record
- Integration into 60-day Service Plan reviews documented
- Comparison across assessments included in step-down evaluations
- Copy provided to SSCC/DFPS caseworker

**Cross-Reference:** FC3-01.1 Individual Service Planning Procedure Section 1.7 (CANS 3.0 Implementation)

### 5.3 24/7 Crisis Availability with On-Call Licensed Therapist

**Cross-Reference:** Crisis response per **FC-04 Crisis Management Policy** and **FC-04.1 Crisis Management Procedure Section 6**

**CRITICAL REQUIREMENT (T3C Blueprint p.138):** On-Call Licensed Therapist must be contacted for ALL crises involving Treatment Foster Family Care children.

| Crisis Component | Requirement | Reference |
|-----------------|-------------|-----------|
| On-Call Licensed Therapist | 24/7 availability; contact for ALL crises | T3C Blueprint p.138; FC-04.1 Section 6.2 |
| Psychiatric Emergency | Specialized protocols for suicidal ideation, psychosis, severe dissociation | FC-04.1 Section 6.4 |
| 60-Day Review Integration | Crisis patterns documented and analyzed at each review | FC-04.1 Section 6.5 |

**Staff Training:** Per **FC-16 Staff and Caregiver Training Policy Section 4.6**, all staff serving TFFC population must complete clinical components of 20-hour specialized training including high-acuity crisis response.

### 5.4 Human Trafficking Prevention

**Cross-Reference:** Per **FC-HT-01 Human Trafficking Prevention Policy** and **Human Trafficking Prevention Training Implementation Plan**.

### 5.5 Documentation and STAR Health Coordination

**Cross-Reference:** Documentation and service coordination per **FC-STAR-01.1 STAR Health Coordination Procedure** and **FC10-01 Physical and Mental Health Care Policy**.

### 5.6 Family Engagement

**Cross-Reference:** Family engagement per **FC7-01 Family Connections and Engagement Policy** with intensive family therapy as clinically indicated.

### 5.7 Educational Coordination

**Cross-Reference:** Educational coordination per **FC-T3C-01** with enhanced support for children with behavioral challenges in educational settings.

---

## 6. STAFFING REQUIREMENTS AND RATIOS

### 6.1 Administrative Leadership

**Cross-Reference:** Leadership qualifications per **FC-T3C-01 T3C Basic Foster Family Home Support Services Policy** (same as Mental & Behavioral Health).

| Position | Requirements |
|----------|--------------|
| **LCPAA** | Full-time, dedicated to single CPA |
| **Program Director** | May serve as LCPAA; bachelor's degree OR 5 years residential experience |
| **Treatment Director** | Must meet T3C Blueprint qualifications; responsible for clinical oversight and 60-day confirmations |

### 6.2 Staff-to-Child Ratios (Intensive)

| Position | Treatment Foster Care Ratio | Mental Health Ratio (comparison) |
|----------|----------------------------|----------------------------------|
| Case Manager | **1:6** | 1:15 |
| Licensed Therapist | **1:11** | 1:14 |
| Behavior Support Specialist/Mentor | **1:6** | 1:15 |
| Crisis Management Staff | 1:25 | 1:25 |
| Aftercare Case Manager | 1:25 | 1:25 |

### 6.3 Required Infrastructure Functions

**Cross-Reference:** Infrastructure functions per **FC-T3C-01 Section 6** including all standard functions plus enhanced clinical infrastructure for Treatment Foster Care.

### 6.4 Specialized Intermittent Alternative Care

**Package-Specific Requirement:** Dedicated program with **1 skilled Intermittent Alternative Care Caregiver for every 20 children** receiving Treatment Foster Family Care to ensure caregiver wellness.

### 6.5 On-Call Licensed Therapist Requirement

**T3C Blueprint p.138 Requirement:** Child Placing Agency must ensure on-call access to a Licensed Therapist who is trained in the CPA's Evidence-informed Treatment Model.

**Implementation:** Treatment Director coordinates on-call coverage through:
- Contracted therapists with Superior HealthPlan/STAR Health network credentialing
- CPA staff therapists with on-call rotation
- Treatment Director serving as clinical backup

**Cross-Reference:** See **FC-04.1 Crisis Management Procedure Section 6.2** for On-Call Licensed Therapist protocols.

### 6.6 Job Description Cross-References

Staff fulfilling these roles must have package-specific responsibilities incorporated into their job descriptions:

| Role | Job Description Section | Key TFFC Responsibilities |
|------|------------------------|---------------------------|
| Case Manager | Section G: TFFC Coordination | 60-day reviews; 1:6 ratio; on-call therapist coordination; step-down planning |
| Crisis Management Staff | Package-Specific Protocols | On-call therapist contact REQUIRED; psychiatric emergency response |
| Treatment Director | TFFC Oversight Section | 60-day review leadership; on-call therapist coordination; dual-signature confirmation |
| Behavior Support Specialist | Package Variation Section | 1:6 intensive ratio; behavior plan implementation |
| Aftercare Case Manager | Package-Specific Aftercare | Weekly contact first month; extended monitoring |

**Cross-Reference:** See **Job Description Targeted Additions - SU/STASS/TFFC** for complete duty specifications.

### 6.7 Hours of Operation

Admissions and placement staff on-call/available **365 days per year, 24 hours per day**.

---

## 7. FOSTER FAMILY HOME REQUIREMENTS

### 7.1 Dual Credentialing Requirement

**CRITICAL:** Foster Family Home AND Child Placing Agency must be Credentialed for BOTH:
- T3C Treatment Foster Family Care Support Services Package
- T3C Basic Foster Family Home Support Services Package

This allows step-down to Basic Package in same home when appropriate.

### 7.2 Credentialing Process

**Cross-Reference:** Credentialing follows **FC-CRED-01.1 Foster Family Home Credentialing Procedure** with the following package-specific additions.

### 7.3 Additional Training Requirements

**Per TAC §749.863(c):** Caregivers must complete **20 hours additional training** for Treatment Foster Care beyond standard foster parent training, including:
- Managing high-acuity behaviors
- Implementing intensive behavior support plans
- Crisis intervention and de-escalation
- Supporting concentrated therapeutic treatment
- Recognizing and responding to psychiatric emergencies

**Cross-Reference:** Training per **FC-16 Staff and Caregiver Training Policy Section 4.6** which specifies:
- **20-hour specialized TFFC training** (required before first TFFC placement) per TAC §749.863(c)
- 8-hour annual specialized training
- Topics: high-acuity behavior management, psychiatric emergency recognition, intensive treatment team participation, step-down transition support

Foster parents must also complete training on crisis response per **FC-04 Crisis Management Policy** with emphasis on On-Call Licensed Therapist coordination.

### 7.4 Caregiver Competencies

Foster Family Home Caregivers must demonstrate ability to:
- Attend multiple meetings per week
- Respond immediately when child is in crisis
- Participate in therapy with child as needed
- Implement behavior support plans consistently
- Maintain structured therapeutic environment

### 7.5 Capacity Limitation

**Maximum 2 children in foster care** per home due to intensity of services.

### 7.6 Awake Night Supervision

Awake night supervision required in foster homes with 7 or more children, or as clinically indicated for specific children with nighttime safety concerns.

---

## 8. QUALITY ASSURANCE AND CONTINUED STAY GUIDELINES

### 8.1 60-Day Continued Stay Reviews

**Cross-Reference:** Continued Stay Reviews follow **FC-CSR-01.1 Continued Stay Review Procedure** with enhanced 60-day frequency and dual-signature requirement.

**CRITICAL REQUIREMENT:** At each 60-day Service Plan review, considering most recent CANS 3.0:

| Reviewer | Required Confirmation |
|----------|----------------------|
| **Program Director** | Written confirmation that child continues to meet criteria and less-restrictive placement is not appropriate |
| **Treatment Director** | Written confirmation that child is benefitting from Treatment Model and less-restrictive placement is not appropriate |

**BOTH signatures required.** Confirmation must be documented in case record and copy provided to SSCC/DFPS within **15 business days** of review.

### 8.2 Continued Stay Criteria

Quality Assurance and Continued Stay Guidelines include:
- Ongoing adjustment based on CANS 3.0 and Service Plan
- Primary admission reason continues or replaced with aligned service needs
- Services support safety, well-being, and permanency
- Less-restrictive placement confirmed inappropriate at each 60-day review
- Foster home maintains Treatment Foster Care credential

### 8.3 Desired Individual Outcomes

**Cross-Reference:** Outcome tracking per **FC-17 Continuous Quality Improvement Policy** and **CQI Plan** with treatment-specific metrics.

---

## 9. AFTERCARE SERVICES REQUIREMENTS

### 9.1 Mandatory 6-Month Aftercare

**CRITICAL:** Treatment Foster Family Care REQUIRES 6 months of Aftercare Services.

**Cross-Reference:** Aftercare follows **FC-AF-01 Aftercare Services Policy** with the following package-specific enhanced schedule:

| Period | Minimum Contact Frequency |
|--------|--------------------------|
| **Month 1** | **Weekly** (in-person, phone, or video) |
| **Months 2-6** | **Twice monthly** (in-person, phone, or video) |

Additional ad-hoc meetings and referrals as needed.

### 9.2 Aftercare Components

Per **FC-AF-01 Aftercare Services Policy**:
- Case management support
- Monitoring of step-down adjustment
- Crisis intervention as needed
- Coordination with receiving placement
- Connection to ongoing services

### 9.3 Aftercare Funding

Built into daily foster care rate; no separate payment for aftercare.

### 9.4 Aftercare Documentation

**Cross-Reference:** Per **FC-AF-01 Aftercare Services Policy**, monthly reports to SSCC/DFPS documenting all contacts, referrals, and case management support.

---

# PROCEDURE SECTIONS

## 10. RELATIONSHIP BETWEEN POLICY AND PROCEDURE

This procedure section implements the governing principles established in Sections 1-9 above and integrates with existing Refuge House procedures. Package-specific modifications are noted explicitly.

---

## 11. ADMISSION AND PLACEMENT PROCEDURES

**Cross-Reference:** This section implements **FC1-01.1 Admission Screening Procedure**, **FC2-01.1 Admission Assessment Procedure**, and **FC6-01 Admission Matching & Criteria Policy** with Treatment Foster Care requirements.

### 11.1 Clinical Screening

**Per FC1-01.1 Admission Screening Procedure** with Treatment Director clinical review:

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| Intake Specialist | 1. Receive referral per FC1-01.1 2. Initial screening for Treatment Foster Care eligibility 3. Forward to Treatment Director for clinical review | Within 2 hours of referral | T3C Blueprint p.141-142; FC1-01.1 |
| Treatment Director | 1. Review clinical documentation 2. Verify DSM-5 diagnosis 3. Confirm 2+ additional criteria met 4. Document clinical appropriateness 5. Approve or redirect | Within 4 hours of screening | T3C Blueprint p.141-142 |

### 11.2 Placement Matching

**Per FC6-01 Admission Matching & Criteria Policy** with dual credentialing verification:

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| Intake Specialist | 1. Verify homes with BOTH Treatment Foster Care AND Basic credentials 2. Confirm home has capacity (max 2 children) 3. Match intensive needs with home capabilities | Within 4 hours of clinical approval | T3C Blueprint p.143; FC6-01 |

### 11.3 Admission Documentation

**Per FC2-01.1 Admission Assessment Procedure** with clinical enhancement:

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| Case Manager | 1. Complete admission assessment per FC2-01.1 2. Document DSM-5 diagnosis and criteria met 3. Initiate intensive service plan 4. Begin step-down planning | Within 24 hours of placement | TAC §749.1301; FC2-01.1 |

---

## 12. SERVICE PLANNING PROCEDURES

**Cross-Reference:** This section implements **FC3-01.1 Individual Service Planning Procedure** with 60-day review cycle and step-down focus.

### 12.1 Initial Service Plan Development

**Per FC3-01.1 Individual Service Planning Procedure** with Treatment Foster Care enhancements:

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| Case Manager | 1. Develop Service Plan per FC3-01.1 2. Include step-down goals from admission 3. Document therapy frequency with clinical justification 4. Include crisis safety planning | Within 30 days of admission | TAC §749.1301; FC3-01.1 |
| Treatment Director | 1. Review and approve Service Plan 2. Verify clinical appropriateness of interventions 3. Approve therapy frequency | Within 5 days of draft completion | T3C Blueprint p.143-144 |

### 12.2 60-Day Service Plan Reviews

**Package-Specific Modification:** Reviews every 60 days (vs. 90 days for Mental Health per FC3-01.1)

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| Service Planning Team | 1. Review treatment progress 2. Update Service Plan 3. Assess step-down readiness 4. Initiate Continued Stay Review per Section 13 | Every 60 days | T3C Blueprint p.145 |

---

## 13. 60-DAY CONTINUED STAY REVIEW PROCEDURES

**Cross-Reference:** This section implements **FC-CSR-01.1 Continued Stay Review Procedure** with 60-day frequency and dual-signature requirement.

### 13.1 Review Preparation

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| Case Manager | 1. Compile review documentation per FC-CSR-01.1 2. Gather therapy notes and progress indicators 3. Obtain current CANS 3.0 results 4. Schedule review meeting 5. Prepare Enhanced Continued Stay Confirmation Form | 7 days before 60-day review date | T3C Blueprint p.145; FC-CSR-01.1 |

### 13.2 Dual Confirmation Process

**CRITICAL:** Both signatures required per T3C Blueprint p.145:

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| Treatment Director | 1. Review clinical progress 2. Complete Treatment Director section confirming: "Child continues to require Treatment Foster Family Care based on clinical indicators" and "Child is benefitting from Treatment Model" 3. Document why less-restrictive placement is insufficient 4. Sign and date | At 60-day review meeting | **CRITICAL: T3C Blueprint p.145** |
| Program Director | 1. Review administrative indicators 2. Complete Program Director section confirming: "Child continues to meet criteria" and "Less-restrictive placement is not appropriate" 3. Sign and date | Within 24 hours of Treatment Director signature | **CRITICAL: T3C Blueprint p.145** |

### 13.3 Documentation Submission

**Per FC-CSR-01.1 Continued Stay Review Procedure:**

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| Case Manager | 1. Compile complete submission package with BOTH signatures 2. Include Service Plan T3C Supplement 3. Submit to SSCC/DFPS 4. Document submission in case record 5. Confirm receipt | **Within 15 business days of 60-day review** | **CRITICAL DEADLINE: T3C Blueprint p.145** |

### 13.4 Crisis Pattern Integration in 60-Day Reviews

**Cross-Reference:** This requirement aligns with **FC-04.1 Crisis Management Procedure Section 6.5**

At each 60-day Continued Stay Review, the following crisis analysis must be included:

| Review Component | Content | Responsible Party |
|-----------------|---------|-------------------|
| Crisis Incident Summary | All crises during review period | Case Manager |
| Pattern Analysis | Frequency, triggers, intensity trends | Case Manager/Treatment Director |
| Intervention Effectiveness | What worked; what didn't | Treatment Director |
| Step-Down Implications | Crisis patterns impact on readiness | Treatment Director |

**Step-Down Readiness Indicators Based on Crisis Patterns:**

| Pattern | Implication |
|---------|-------------|
| Significant reduction in frequency/intensity | May indicate step-down readiness |
| Stable pattern without improvement | Treatment adjustment needed before step-down |
| Increasing frequency/intensity | Step-down not appropriate; intensify treatment |
| Crisis-free for extended period | Strong step-down readiness indicator |

**Documentation:** Use TFFC 60-Day Crisis Pattern Analysis Template.

### 13.5 Step-Down Determination

At each 60-day review, assess whether child can transition to:
- Same home under Basic Foster Family Home Package (if dually credentialed and child stable)
- Different home under Basic or other appropriate Service Package
- Permanency placement

---

## 14. THERAPEUTIC SERVICES PROCEDURES

**Cross-Reference:** This section implements therapeutic service requirements coordinated through **FC-STAR-01.1 STAR Health Coordination Procedure**.

### 14.1 Therapy Coordination

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| Case Manager | 1. Coordinate therapy services per FC-STAR-01.1 2. Ensure weekly minimum individual therapy 3. Arrange family therapy as indicated 4. Document all therapy sessions | Ongoing | T3C Blueprint p.143-144; FC-STAR-01.1 |
| Treatment Director | 1. Provide clinical oversight 2. Review therapy progress 3. Adjust therapy frequency as clinically needed | Monthly minimum | T3C Blueprint p.144 |

### 14.2 Behavior Support

**Per FC-MH-01 Mental & Behavioral Health Support Services Policy** with intensive enhancement:

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| BSS/Mentor | 1. Implement behavior support plan 2. Provide direct behavioral coaching 3. Support foster family with interventions 4. Document behavioral data | Per Service Plan (1:6 ratio) | T3C Blueprint p.144 |

---

## 15. DISCHARGE AND STEP-DOWN PROCEDURES

**Cross-Reference:** This section implements **FC14-01 Discharge and Permanency Planning Policy** with time-limited step-down focus.

### 15.1 Step-Down Readiness Assessment

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| Treatment Director | 1. Review treatment progress indicators 2. Assess behavioral stability 3. Evaluate crisis frequency reduction 4. Recommend step-down when appropriate | At each 60-day review | T3C Blueprint p.146; FC14-01 |

### 15.2 Step-Down Options

Per **FC14-01 Discharge and Permanency Planning Policy**:

| Step-Down Option | Requirements |
|------------------|--------------|
| Same home - Basic Package | Home dually credentialed; child stable; foster family willing to continue |
| Different home - Basic Package | Appropriate home identified; transition planning completed |
| Different home - Other Package | Alternative Service Package appropriate; transition coordinated |
| Permanency placement | Permanency resource identified; preparation completed |

### 15.3 Transition Documentation

**Per FC14-01 Discharge and Permanency Planning Policy:**

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| Case Manager | 1. Prepare comprehensive transition summary per FC14-01 2. Include all treatment progress documentation 3. Transfer effective behavioral interventions 4. Coordinate warm handoff to receiving placement 5. Initiate aftercare services | Before discharge | TAC §749.1361; FC14-01 |

---

## 16. AFTERCARE SERVICES PROCEDURES

**Cross-Reference:** This section implements **FC-AF-01 Aftercare Services Policy** with Treatment Foster Care enhanced schedule.

### 16.1 Aftercare Initiation

**Per FC-AF-01 Aftercare Services Policy:**

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| Aftercare Case Manager | 1. Receive case transfer from Case Manager 2. Contact receiving placement within 48 hours 3. Establish aftercare schedule per package requirements 4. Document initial aftercare plan | Within 5 days of discharge | T3C Blueprint p.147; FC-AF-01 |

### 16.2 Enhanced Contact Schedule

**Package-Specific Schedule:**

| Period | Frequency | Method |
|--------|-----------|--------|
| Month 1 | **Weekly** | In-person, phone, or video |
| Months 2-6 | **Twice monthly** | In-person, phone, or video |
| As needed | Ad-hoc | Per crisis or emerging need |

### 16.3 Aftercare Monitoring and Documentation

**Per FC-AF-01 Aftercare Services Policy:**

| Who | How | When | Regulatory Reference |
| :---- | :---- | :---- | :---- |
| Aftercare Case Manager | 1. Document all contacts per FC-AF-01 2. Monitor step-down adjustment 3. Provide crisis intervention as needed 4. Prepare monthly reports to SSCC/DFPS | Per contact schedule | T3C Blueprint p.147; FC-AF-01 |

---

# TREATMENT MODEL AND CQI

## 17. TREATMENT MODEL APPLICATION

**Cross-Reference:** TBRI® implementation per **TBRI Treatment Model Executive Summary** and **Executive Summary - TBRI Implementation Through T3C Master Policy Framework**.

For Treatment Foster Family Care, intensive clinical interventions are integrated with TBRI principles:
- Concentrated therapeutic services addressing severe trauma impacts
- Immediate clinical access for behavioral crises
- Intensive behavior support with trained specialists
- Structured therapeutic environment in family setting

---

## 18. CONTINUOUS QUALITY IMPROVEMENT

**Cross-Reference:** CQI follows **FC-17 Continuous Quality Improvement Policy** and **CQI Plan** with treatment-specific metrics.

### 18.1 Treatment Foster Care Quality Metrics

| Metric | Target | Measurement |
|--------|--------|-------------|
| 60-day reviews completed on time | 100% | Monthly tracking |
| Dual signatures obtained | 100% | Each review |
| 15-day submission deadline met | 100% | Monthly tracking |
| Successful step-down within 365 days | 90% | Quarterly review |
| Aftercare contacts completed | 100% | Monthly tracking |
| Post-discharge stability at 90 days | 85% | Quarterly review |

---

# REFERENCE SECTIONS

## 19. DEFINITIONS

**Dual Credentialing:** Requirement that CPA and foster home maintain credentials for BOTH Treatment Foster Family Care and Basic Foster Family Home packages.

**Step-Down:** Planned transition from Treatment Foster Family Care to less-restrictive placement based on treatment progress.

**Treatment Foster Family Care:** Highest level of clinical intervention in family setting for children with DSM-5 diagnosis plus two additional criteria, requiring intensive, time-limited (365-day maximum) therapeutic services.

---

## 20. REGULATORY REFERENCES

- **T3C Blueprint Standards:** Pages 136-147, T3C Treatment Foster Family Care Support Services
- **Texas Family Code Section 264.1073**
- **Texas Administrative Code Rule §700.1335**
- **TAC Chapter 749:** Minimum Standards for Child Placing Agencies
- **TAC §749.863(c):** Treatment Foster Care training requirements
- **RCC Contract Requirements:** Contract Terms 7, 8, 10

---

## 21. RELATED POLICIES AND PROCEDURES

### Core T3C Framework:
- **FC-T3C-01** T3C Basic Foster Family Home Support Services Policy
- **FC-T3C-01.1** T3C Basic Foster Family Home Support Services Procedure

### Clinical Framework:
- **FC-MH-01** Mental & Behavioral Health Support Services Policy

### Admission and Assessment:
- **FC1-01.1** Admission Screening Procedure
- **FC2-01.1** Admission Assessment Procedure
- **FC6-01** Admission Matching & Criteria Policy

### Service Planning:
- **FC3-01.1** Individual Service Planning Procedure

### Continued Stay Reviews:
- **FC-CSR-01.1** Continued Stay Review Procedure

### Health and STAR Health:
- **FC10-01** Physical and Mental Health Care Policy
- **FC-STAR-01.1** STAR Health Coordination Procedure

### Discharge and Aftercare:
- **FC14-01** Discharge and Permanency Planning Policy
- **FC-AF-01** Aftercare Services Policy

### Credentialing and Training:
- **FC-CRED-01.1** Foster Family Home Credentialing Procedure
- **FC-16** Caregiver Training Policy

### Quality and Compliance:
- **FC-17** Continuous Quality Improvement Policy
- **CQI Plan**

### Crisis Management:
- **FC-04** Crisis Management Policy
- **FC-04.1** Crisis Management Procedure (Section 6: Treatment Foster Family Care)
- **FC-16** Staff and Caregiver Training Policy (Section 4.6: TFFC Training)

### Other:
- **FC-HT-01** Human Trafficking Prevention Policy
- **TBRI Treatment Model Executive Summary**

---

## 22. FORMS AND ATTACHMENTS

- Treatment Foster Care Admission Screening Checklist
- DSM-5 Diagnosis and Criteria Verification Form
- Enhanced Continued Stay Confirmation Form (60-Day) - **BOTH signatures required**
- Service Plan - Treatment Foster Care Supplement
- Step-Down Readiness Assessment Tool
- Aftercare Services Plan Template
- Monthly Aftercare Report Template

**Crisis Management Forms (per FC-04.1):**
- Treatment Foster Family Care Psychiatric Emergency Checklist
- TFFC 60-Day Crisis Pattern Analysis Template
- On-Call Licensed Therapist Consultation Log

---

## DOCUMENT CONTROL

| Version | Date | Author | Changes |
|---------|------|--------|---------|
| 1.0 | December 2025 | Refuge House Leadership | Initial document creation |

---

**COMPLIANCE STATEMENT**

This policy and procedure document demonstrates Refuge House's compliance with T3C Blueprint requirements (April 2025) for T3C Treatment Foster Family Care Support Services, including Texas Family Code Section 264.1073 and TAC Rule §700.1335. This umbrella document integrates with and references existing Refuge House policies and procedures.

---

*This umbrella policy aggregates T3C Treatment Foster Family Care Support Services requirements while cross-referencing existing policies and procedures. Package-specific modifications to standard procedures are noted explicitly, particularly the 60-day review cycle with dual-signature requirement and enhanced aftercare schedule.*
